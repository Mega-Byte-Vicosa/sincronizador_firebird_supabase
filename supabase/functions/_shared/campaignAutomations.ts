...
function clienteAtendeRegra(cliente: ClienteAutomacao, tipo: string, hoje: DataCivil) {
  ...
  if (tipo === "clientes_inativos_90_dias") {
    return Boolean(ultimaCompra && compararDatas(ultimaCompra, deslocarDias(hoje, -90)) <= 0);
  }
  if (tipo === "pos_compra_2_dias") return Boolean(ultimaCompra && compararDatas(ultimaCompra, deslocarDias(hoje, -2)) === 0);
  return false;
}
...
async function jaEnviadoNoPeriodo(
  ...
  const desde = campanha.tipo_automacao === "clientes_inativos_90_dias"
  ...
  return (data ?? []).some((envio: { criado_em?: string | null; enviado_em?: string | null; request_payload?: Record<string, unknown> | null }) => {
    if (campanha.tipo_automacao === "pos_compra_2_dias") {
      return envio.request_payload?.ciclo_automacao === cliente.dt_ultcomp?.split("T")[0];
    }
    if (campanha.tipo_automacao === "clientes_inativos_90_dias") return true;
...
export async function processarCampanhasAutomatizadas(
  ...
  let query = supabase.from("tab_campanha").select("id, id_empresa, nome, mensagem, tipo_automacao, empresa_destino, aos_cuidados, campanha_continua, termina_em, data_hora_agendamento, automacao_proxima_execucao_em, automacao_total_envios, automacao_total_erros")
  ...
  for (const raw of data ?? []) {
    ...
    const elegiveis = (clientes ?? []).filter((item) => clienteAtendeRegra(item as ClienteAutomacao, campanha.tipo_automacao, hoje)) as ClienteAutomacao[];
