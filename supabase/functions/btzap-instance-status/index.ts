...
Deno.serve(async (req) => {
  ...
  if (req.method !== "POST" && req.method !== "GET") {
    return jsonResponse({ success: false, message: "Metodo nao permitido." }, 405);
  }
  ...
  try {
    ...
    if (!response.ok) {
      return jsonResponse({
        success: false,
        message: "Nao foi possivel consultar o status da instancia BTZap.",
    ...
    return jsonResponse(
      {
        success: false,
        message: "Erro ao consultar status da BTZap.",
