alter table public.tab_cliente
  add column if not exists permite_campanha boolean not null default false,
  add column if not exists permite_cobranca_aviso boolean not null default true,
  add column if not exists contato_restrito boolean not null default false,
  add column if not exists motivo_restricao text null,
  add column if not exists restrito_em timestamp with time zone null,
  add column if not exists atualizado_em timestamp with time zone not null default now();

create or replace function public.set_tab_cliente_atualizado_em()
returns trigger
language plpgsql
...
drop trigger if exists trg_tab_cliente_atualizado_em on public.tab_cliente;
create trigger trg_tab_cliente_atualizado_em
before update on public.tab_cliente
for each row execute function public.set_tab_cliente_atualizado_em();
